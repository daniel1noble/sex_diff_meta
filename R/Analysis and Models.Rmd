...
# let's just run it anyway because we said we would
# 1. mating system and overall mean differences / variability differences for birds only
  ...
  # run the models 
#SMD
  MLMR_mods_mate_bird_SMD <- rma.mv(SMD_yi_flip ~ mating_system -1, V = SMD_vi, 
                                          random = list(~1|study_ID, ~1|spp_name_phylo, ~1|obs), 
                                          R = list(spp_name_phylo=phylo_vcv_bird), control=list(optimizer="optim"), 
                                          test = "t", data = pers_new_bird)

#lnCVR
  MLMR_mods_mate_bird_lncvr <- rma.mv(CVR_yi ~ mating_system -1, V = CVR_vi, 
                                            random = list(~1|study_ID, ~1|spp_name_phylo, ~1|obs), 
                                            R = list(spp_name_phylo=phylo_vcv_bird), control=list(optimizer="optim"), 
                                            test = "t", data = pers_new_bird)
  
# now look at how mating system moderates boldness in birds
   # subset dataset
  ...
  # use same phylo_vcv_bird as previous model
...
# 3. Trim-and-fill
  # uses funnel asymmetry to add more points (randomly or in another manner) and provides a revised overall mean for models 
    # SMD:
    tf_SMD <- trimfill(smd_mods[1]) # run models seperately for this method I think
    
    # lnCVR:
    tf_lncvr <- trimfill(lnCVR_mods)
```
