...
write.csv(latency_all, file = "pers_latency.csv", row.names = FALSE)  
 ...
 # Remove Pruitt papers from dataset as they have been flagged for data duplication issues by coauthors ???
```
...
# First, we need to subset our pers dataset by taxo group to drop the unwanted levels. Let's start with mammals because it's the easiest one
    ...
    filter(!personality_trait == "sociality") %>%
...
MLMR_models_pers_SSD_mammal <- meta_model_fits(pers_new_mammal, phylo_vcv[4], type = "pers_SSD")
...
# 4. INVERTS
  ...
  # rerun models
#SMD
  MLMR_mods_pers_SSD_invert_SMD <- rma.mv(SMD_yi_flip ~ -1 + personality_trait*SSD_index, V = SMD_vi, 
                                          random = list(~1|study_ID, ~1|spp_name_phylo, ~1|obs), 
...
smd_mods_D <- ob[[1]]
  ...
  #extract PR
  MLMA_estimates_SMD_D_0.5 <- plyr::ldply(lapply(smd_mods_D_0.3, 
                           function(x) print(mod_results(x, mod = "Int")))) 
  MLMA_estimates_lnCVR_D_0.5 <- plyr::ldply(lapply(lnCVR_mods_D_0.3, 
                              function(x) print(mod_results(x, mod = "Int"))))
  ...
  #pers_trait_D <- fit_MLMA_reg_models_personality_trait_D(pers_new, phylo_vcv, D_matrices_0.8)   
  
  smd_pers_mods_D <- pers_trait_D["SMD",]
  lnCVR_pers_mods_D <- pers_trait_D["lnCVR",] 
...
MLMA_models_D <- meta_model_fits_D(pers_new, phylo_vcv, D_matrices_0.8, type = "int") # elements of R must have dimension names
...
# how many effect sizes are scores?
  ...
  # as a check, let's rerun our 3 main models with score data removed for inverts
     ...
     # filter dataset by invert taxa group and filter out scores 
    invert_noscore <- as.data.frame(pers_new %>%
    filter(taxo_group == "invertebrate" & !score == "score")) 
  
   # rerun intercept model, pers_trait model and SSD models
...
# make a matrix of effect sizes (n) for each species for each personality trait to add to our plot!
  ...
  # add the heatmap data to our plot
     invertplot <- gheatmap(p5, invert_matrix, offset = 30, width = 1.5, 
         low = "white", high = "mediumseagreen", color=NULL, 
         colnames_position="bottom", 
