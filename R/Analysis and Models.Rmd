...
# Intercept models
   # break these up for each taxo group
    birds_fit_pers_trait_MLMAmodel_D <- function(data, phylo_vcv, D){

          lnCVR <- mapply(function(x,y,z) metafor::rma.mv(CVR_yi ~ personality_trait-1, V = CVR_vi, 
            random = list(~1|study_ID, ~1|spp_name_phylo, ~1|obs), 
            R = list(spp_name_phylo=y, obs = z), 
            data = x), x = pers_new_bird, y = phylo_vcv, z = D_matrices_0.8)
  ...
  birds_fit_pers_trait_MLMAmodel_D(pers_new_bird, phylo_vcv[1], D_matrices_0.8[1]) #error
 
   split_taxa <- split(pers_new, pers_new$taxo_group)
    smd_pers_mods_D <- fit_pers_trait_MLMAmodel_D["SMD",]
  lnCVR_pers_mods_D <- fit_pers_trait_MLMAmodel_D["lnCVR",]  
