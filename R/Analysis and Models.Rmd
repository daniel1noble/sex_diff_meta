...
# Just select the relevant columns for now to make life easier
  pers_new <- pers %>% 
          select(study_ID, year, species_name, SSD_index, taxo_group, data_type, personality_trait, male_n, male_mean, m_SE_to_SD, female_n, female_mean, f_SE_to_SD, depend, directionality, spp_name_phylo, mating_system, age, population, study_environment, study_type, measurement_type)
...
group_by(taxo_group, personality_trait) %>%
filter(!is.na(SSD_index))%>%
summarise(N_ssd = length(unique(SSD_index)), N_spp = length(unique(spp_name_phylo)), N_studies = length(unique(study_ID))))
...
# First, we need to subset our pers dataset by taxo group to drop the unwanted levels. Let's start with mammals because it's the easiest one
    ...
    filter(!personality_trait == "activity" & !personality_trait == "sociality") %>%
...
fit_MLMA_SSD_mammal <- function(data, phylo_vcv){
    
  #SMD
  MLMR_mods_pers_SSD_mammal_SMD <- rma.mv(SMD_yi_flip ~ -1 + personality_trait*SSD_index, V = SMD_vi, 
...
#lnCVR
  MLMR_mods_pers_SSD_mammal_lncvr <- rma.mv(CVR_yi ~ -1+ personality_trait*SSD_index, V = CVR_vi, 
                                            random = list(~1|study_ID, ~1|spp_name_phylo, ~1|obs), 
                                            R = list(spp_name_phylo=phylo_vcv_mammal), control=list(optimizer="optim"), 
                                            test = "t", data = pers_new_mammal)
}

fit_MLMA_SSD_mammal(pers_new_mammal, phylo_vcv[4])
...
#lnCVR
  MLMR_mods_pers_SSD_fish_lncvr <- rma.mv(CVR_yi ~ study_type, V = CVR_vi, 
                                            random = list(~1|study_ID, ~1|spp_name_phylo, ~1|obs), 
                                            R = list(spp_name_phylo=phylo_vcv_fish), control=list(optimizer="optim"), 
                                            test = "t", data = pers_new_fish)
...
# 1. funnel plot for all our effect sizes?
  ...
  # funnels with full personality model
    # SMD
    plot(MLMR_models_pers_trait[1], qq= TRUE)
...
# 2. Egger's test - regression test
  ...
  # looking at model residuals for our intercept models
# SMD
    #coef(smd_mods) doesn't give us anything - might have to run models another way?
...
ggsave("birdphylo.pdf", dpi = 300) #saves how it looks in the plot window
...
p5 <- ggtree(pruned.inverttree, branch.length='none', layout='circular') %<+% invert_data + 
  geom_tippoint(aes(color=SSD_index)) + 
  scale_color_gradient2(midpoint = 0, low = "red3", mid = "seashell2", high = "deepskyblue2") + 
  geom_tiplab2(align=T, linetype=NA, size=2, offset=15, hjust=0.5) +
  theme(legend.position = "right")
