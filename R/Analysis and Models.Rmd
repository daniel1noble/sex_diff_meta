...
# merge spp_names columns
  pers <- merge(pers,bodysize[,c("species_name", "m_BS", "m_SE", "f_BS", "f_SE", "measure", "SSD_index", "mating_system", "parental_care")],by="species_name", all=TRUE,  no.dups = TRUE)
  summary(pers)
  nrow(pers) #
...
```{r}
    ...
    # Check that all names in the phylogeny are also in the data
      taxa_data_list <- split(pers_new, pers_new$spp_name_phylo)
