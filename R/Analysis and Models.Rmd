...
# Just select the relevant columns for now to make life easier
  pers_new <- pers %>% 
          select(study_ID, year, species_name, SSD_index, taxo_group, data_type, personality_trait, male_n, male_mean, m_SE_to_SD, female_n, female_mean, f_SE_to_SD, depend, directionality, spp_name_phylo, age)
...
# 2. BIRDS
  ...
  # rerun models
#SMD
  MLMR_mods_pers_SSD_bird_SMD <- rma.mv(SMD_yi_flip ~ 1- SSD_index, V = SMD_vi, 
                                          random = list(~1|study_ID, ~1|spp_name_phylo, ~1|obs), 
                                          R = list(spp_name_phylo=phylo_vcv_bird), control=list(optimizer="optim"), 
                                          test = "t", data = pers_new_bird)

#lnCVR
  MLMR_mods_pers_SSD_bird_lncvr <- rma.mv(CVR_yi ~ 1- SSD_index, V = CVR_vi, 
                                            random = list(~1|study_ID, ~1|spp_name_phylo, ~1|obs), 
                                            R = list(spp_name_phylo=phylo_vcv_bird), control=list(optimizer="optim"), 
                                            test = "t", data = pers_new_bird)
...
# 8. mating system and 9. parental care - models weren't very informative/lacked data/difficult to run so these have been removed from analysis

# 10. Age
Also recorded whether individuals were adults or juveniles (not sexually mature). Since we might expect that sex differences in behaviours (particularly behaviours related to mate choice or mating success) would manifest once individuals are sexually mature, it would sprobably be a good idea to see if age moderates sex differences in personalities. Let's have a look at this in a secondary model with age as a moderator:

```{r}
# Let's see what we have to work with
data.frame(pers_new %>%
group_by(taxo_group, personality_trait, age) %>%
summarise(N_spp = length(unique(spp_name_phylo)), N_studies = length(unique(study_ID))))

# some groups have only 1 effect size or 1 species, we can probably remove those 

# we can modify our mating system model to look at age as a categorical moderator

fit_MLMA_reg_models_personality_trait_age  <- function(data, phylo_vcv){
          lnCVR <- metafor::rma.mv(CVR_yi ~ -1 + personality_trait + age, V = CVR_vi, random = list(~1|study_ID, ~1|spp_name_phylo, ~1|obs), R = list(spp_name_phylo=phylo_vcv), control=list(optimizer="optim"), test = "t", data = data)

            SMD <- metafor::rma.mv(SMD_yi_flip ~ -1 + personality_trait + age, V = SMD_vi, random = list(~1|study_ID, ~1|spp_name_phylo, ~1|obs), R = list(spp_name_phylo=phylo_vcv), control=list(optimizer="optim"), test = "t", data = data) 
...
# model:
 MLMR_models_pers_age <- meta_model_fits(pers_new, phylo_vcv, type = "pers_mate")

# Extract the SMD and lnCVR results
  smd_mods_pers_age <- MLMR_models_pers_age["SMD",]
  lnCVR_mods_pers_age <- MLMR_models_pers_age["lnCVR",]
  
# these models have adult age as the intercept, probably need to figure out how to swap the levels and compare them
...
# 0.8 rho:   
  ...
  # NOTE: Lauren –Have a look and make sure these matrices are correct. You should see correlations for numbers within studies that match. Usually these are blocks of effects along the diagonal. IF you have ones far away form the signal, then you need to check these. Here is an example with reptiles
   ...
   fit_int_MLMAmodel_D(pers_new, phylo_vcv, D_matrices_0.5) # error - R not square matrix?
