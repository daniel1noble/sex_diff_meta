...
# using Dan's code to extract estimates from SSD models
```{r}
# define some things first
model = MLMR_models_pers_SSD
mod_cat = "personality_trait"
mod_cont = "SSD_index"
...
get_pred_est_cont <- function (model, mod_cat, mod_cont, type = c("mean", "zero")) {
	...
	# Continuous moderators
		  ...
		  if(type == "mean"){
			  if(len_cont == 1){  	
			  	means_cont <- mean(model$X[,mod_cont, drop = FALSE])
			  	names(means_cont) <- mod_cont
			  }
			  if(len_cont > 1){
				means_cont <- colMeans(model$X[,mod_cont, drop = FALSE])
			  }
		}
...
get_pred_est_cont(MLMR_models_pers_SSD_mammal, pers_new_mammal$personality_trait, pers_new_mammal$SSD_index) #Error - x must be an array of 2 dimensions
...
# personality SMD
# Bird SMD
bird_SMD <- orchard_plot(smd_mods_pers[[1]], mod = "personality_trait", xlab = "Standardised mean difference", angle = 45, alpha = 0.5, transfm = "tanh") #tanh pulls in outliers to make the graph easier to see (a bit dodgy?)
# Fish lnCVR
...
p <- ggtree(pruned.birdtree, branch.length='none', layout='circular') %<+% bird_data + 
  ...
  geom_tiplab2(align=T, linetype=NA, size=2, offset=10, hjust=0.5) 
...
gheatmap(p, birds_matrix, offset = 18, 
...
dev.new(width=5,height=7,noRStudioGD = TRUE) #this is in inches, I think you can change it to pixels but not cms?
```
