...
```{r}
# set up a new dataframe with spp_name_phylo being a unique row name
  # Merge spp_names columns like we did initially, but this time bodysize is our x dataset
  pers_tree <- merge(x = bodysize,
                y = pers[,c("species_name", "spp_name_phylo", "personality_trait")],
                by="species_name", all.x=TRUE,  no.dups = TRUE) #duplicates species names, I just want a list of spp_names_phylo
  ...
  pers_new %>%
      group_by(taxo_group, spp_name_phylo) %>%
      summarise(n = n())    
  
  # SSD values for each species
  pers_new %>%
      group_by(taxo_group, spp_name_phylo, SSD_index)
...
# setting up the basic tree structure
  # load dataset and tree, set node colours
info <- read.csv("info.csv")
tree <- read.tree("tree.nwk")
cols <- c(NA ='black', male-biased ='purple2', female-biased ='skyblue2')

 # tree structure (cladogram, circular)
p <- ggtree(tree, branch.length='none', layout='circular') %<+% info + 
