...
pacman::p_load(knitr, metafor, dplyr, kableExtra, tidyverse, rotl, phytools, GGally)
```
...
meta_model_fits <- function(data, phyloVCV_list, formula){
        taxa_list <- split(pers_new, pers_new$taxo_group.x)


        for(i in 1:length(taxa_list)){
          lnCVR <- metafor::rma.mv(CVR_yi ~ as.formula(formula), V = yi, random = list(~1|study_ID, ~1|species_name, ) )
