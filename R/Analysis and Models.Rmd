...
# checking lnCVR outliers
 ...
 pers_new %>%
   filter(study_ID == "P041") # each datapoint from this study is at the top of the funnel - remove and see if model runs?
 
 # remove P041
 pers_new <- pers_new %>%
              filter(!study_ID == "P041")
  dim(pers_new) # looks more normal now
...
if(rerun_models == TRUE){
      MLMR_models_pers_mating_system <- meta_model_fits(pers_new, phylo_vcv, type = "pers_mate")
      saveRDS(MLMR_models_pers_SSD, "./output/MLMR_models_pers_mating_system")
    } else{
      MLMR_models_pers_SSD <- readRDS("./output/MLMR_models_pers_mating_system")
    }
  ...
  # Intercept models
    fit_int_MLMAmodel_D <- function(data, phylo_vcv, D){
          ...
          lnCVR <- mapply(function(x,y,z) metafor::rma.mv(CVR_yi ~ 1, V = CVR_vi, 
           ...
           data = x), x = split_taxa, y = phylo_vcv, z = D, SIMPLIFY = FALSE)
   ...
   split_taxa <- split(pers_new, pers_new$taxo_group)
    smd_mods_D <- ob[["SMD"]] #these don't work
...
Here we are adding a moderator term for data that are a) scores, and b) proportions to see if they significantly influence the means of our intercept and pers traits models
```{r}
# ifelse statement for our two columns to make data into binary categories
# scores
pers_new$score <- ifelse(is.na(pers_new$score), 0, 1)

    pers_new$score <- ifelse(pers_new$score == 1, "score", "other")

# proportions    
pers_new$proportion <- ifelse(is.na(pers_new$proportion), 0, 1)

    pers_new$proportion <- ifelse(pers_new$proportion == 1, "proportion", "other")
    
# running simple models first
  # scores
  data.frame(pers_new %>%
 ...
 sensitivity_mod1_score <- meta_model_fits(pers_new, phylo_vcv, type = "score")
  ...
  # same direction but more significant for invert SMD and lnCVR
  
  # proportions
  data.frame(pers_new %>%
  group_by(taxo_group, proportion) %>%
  summarise(n = n())) 
  
  # no reptiles with proportional data, drop those
    pers_new_prop <- as.data.frame(pers_new %>%
  filter(!taxo_group == "reptilia")) 
    
    phylo_vcv_prop <- phylo_vcv[-5]
  
    # model:
 sensitivity_mod2_prop <- meta_model_fits(pers_new_prop, phylo_vcv_prop, type = "proportion")
 
    # Extract the SMD and lnCVR results
  smd_mods_prop <- sensitivity_mod2_prop["SMD",] 
  lnCVR_mods_prop <- sensitivity_mod2_prop["lnCVR",] 

  # fish and inverts have significantly different effect sizes for proportional data than other data (same direction for fish and invert SMD, opposite direction for invert lnCVR)
  #############################
  # let's rerun our 3 main models excluding scores and proportions to see how they change things (expect to see differences for inverts, maybe fish, but also for some personality traits like boldness or aggression)
    
  # additionally, removing scores and proportional data/effect sizes will remove most of the outliers we've flagged in our data, and likely resolve the large smapling variance ratios flagged in the models.
  
  # filter out scores and proportions from dataset
 pers_new <- as.data.frame(pers_new %>%
  filter(!score == "score" & !proportion == "proportion")) 
  
  # check this has been done correctly
  data.frame(pers_new %>%
  group_by(taxo_group, proportion, score) %>%
  summarise(n = n())) #good to go
...
# 1. funnel plot for all our effect sizes?
 ...
 # adding precision to dataset as a moderator term
       pers_new$precision <- 1/sqrt(pers_new$SMD_vi) # use this as a moderator in our full model
