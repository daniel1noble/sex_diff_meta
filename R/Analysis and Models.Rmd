...
funnel(x = pers_new$SMD_yi, vi = pers_new$SMD_vi, yaxis="seinv", pch=as.character(pers_new$study_ID))
```
...
# plot SSD distribution in each taxo group for each pers trait to get an idea of spread
  ggplot(pers_new, aes(x=taxo_group, y=SSD_index, fill=personality_trait)) 
    + geom_boxplot()
...
# 0.8 rho:   
  ...
  meta_model_fits_D <- function(data, phylo_vcv, D, type = c("int", "pers", "pers_SSD")){
    ...
    if(type == "int"){
        model_fits <- mapply(function(x, y) fit_int_MLMAmodel_D(x, y), x = taxa_list, y = phylo_vcv)
    }
...
MLMA_models_D <- meta_model_fits_D(pers_new, phylo_vcv, D_matrices_0.5, type = "int")
