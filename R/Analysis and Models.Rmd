---
title: "Males are not the more variable sex when it comes to personality - Data Analysis and Meta-A Models"
...
# 2. Load datasets
```{r}
# read in pers_new.csv ready to go
  pers_new <- read.csv("./data/pers_new.csv", stringsAsFactors = FALSE)

# OR
# load and work through original datasets
...
# Just select the relevant columns for now to make life easier
  pers_new <- pers %>% select(study_ID, year, species_name, SSD_index, taxo_group, data_type, personality_trait, 
                              male_n, male_mean_conv, male_sd_conv, female_n, female_mean_conv, female_sd_conv, 
                              depend, directionality, spp_name_phylo, mating_system, age, population, study_environment, 
                              study_type, measurement_type)
...
# Our dataset has 1) latency data which has not been corrected for normality (i.e. log-transformed), and 2) proportional data which constrains distributions of variances at high and low values. As such, we need to correct these means and SDs in the dataset before we calculate SMD and lnCVR effect sizes. Scores will be considered as 'normal' data. These corrections should get rid of any outliers.
  
  # 1. converting proportions to logit scale 
 # filter dataset to show only proportional data
  ...
  proportion_all <- convert_propor(pers_proportion$male_mean, pers_proportion$m_SE_to_SD, pers_proportion$female_mean, pers_proportion$f_SE_to_SD)

  # 2. converting latency data to log-latency
 ...
 # both sexes combined in the one function now - save list to object so I can save as dataframe
  latency_all <- convert_latency(pers_latency$male_mean, pers_latency$m_SE_to_SD, pers_latency$female_mean, 
        ...
        # a bunch of NAs for some reason, but running the calculation with the means and SD works so not sure what's wrong here

 # make table into dataframe, try merge() to merge with subset, then merge the 3?
write.csv(proportion_all, file = "pers_proportion.csv", row.names = FALSE)  
write.csv(latency_all, file = "./data/pers_latency.csv", row.names = FALSE)  
    # add in the new columns manually since I've wasted so much time trying to do it in code
...
# how many studies do we have to start with?
  pers_new %>%
  summarise(study = length(unique(study_ID)))
...
2. Scores - cut them out? 
    ...
    Left them in, but removed some outlier scores from a single study
...
Save pers_new dataset so it's ready to go
```{r}
write.csv(pers_new, file = "pers_new.csv", row.names = FALSE)
```
...
```{r}
    ...
    # Check that all names in the phylogeny are also in the data
      ...
      other_groups <- mapply(x = taxa_data_list, 
                             y = trees, 
                             function(x,y) tree_checks(x,y, "spp_name_phylo", type = "checks")) #suddenly getting an error??
...
# Get the combined estimates from them all
  # need to run revised orchard functions to get this #
...
# Rerun SSD models as contrast models:
  # a. activity
  #SMD
...
#lnCVR
  ...
  ###########################  
MLMR_models_pers_SSD_mammal_activity <- meta_model_fits(pers_new_mammal_activity, phylo_vcv[4], type = "pers_SSD")

# Extract the SMD and lnCVR results
    smd_mods_pers_SSD_mammal <- MLMR_models_pers_SSD_mammal["SMD",]
  lnCVR_mods_pers_SSD_mammal <- MLMR_models_pers_SSD_mammal["lnCVR",]
...
# Let's run the model with mating system and overall personality means/variability just to check
  # save output to make life easier
if(rerun_models == TRUE){
...
# 0.8 rho:   
  ...
  # Intercept models - need to run this function first otherwise can't get a proper model output summary
    ...
    # intercept mods - rho=0.8
   ob <- fit_int_MLMAmodel_D(pers_new, phylo_vcv, D_matrices_0.8) # gives an ugly but otherwise useful model summary

   split_taxa <- split(pers_new, pers_new$taxo_group)
    smd_mods_D <- ob[["SMD"]] 
  lnCVR_mods_D <- ob[["lnCVR"]] 
smd_mods_D <- ob[[1]]
  MLMA_estimates_SMD_D <- plyr::ldply(lapply(smd_mods_D, 
                           function(x) print(mod_results(x, mod = "Int")))) 
  MLMA_estimates_lnCVR_D <- plyr::ldply(lapply(lnCVR_mods_D, 
                              function(x) print(mod_results(x, mod = "Int"))))
 
  # intercept mods - rho=0.5
  ob2 <- fit_int_MLMAmodel_D(pers_new, phylo_vcv, D_matrices_0.5)
    split_taxa <- split(pers_new, pers_new$taxo_group)
    smd_mods_D_0.5 <- ob2[["SMD"]] 
  lnCVR_mods_D_0.5 <- ob2[["lnCVR"]] 
  ...
  # intercept mods - rho=0.3
  ob3 <- fit_int_MLMAmodel_D(pers_new, phylo_vcv, D_matrices_0.3)
    split_taxa <- split(pers_new, pers_new$taxo_group)
    smd_mods_D_0.3 <- ob3[["SMD"]] 
  lnCVR_mods_D_0.3 <- ob3[["lnCVR"]] 
  ...
  lnCVR_mods_D__pers_0.8 <- pers_0.8[["lnCVR"]] 
  
  #extract PR
  MLMA_estimates_SMD_pers_D_0.8 <- plyr::ldply(lapply(smd_mods_D_pers_0.8, 
                           function(x) print(mod_results(x, mod = "Int")))) 
  MLMA_estimates_lnCVR_pers_D_0.8 <- plyr::ldply(lapply(lnCVR_mods_D__pers_0.8, 
                              function(x) print(mod_results(x, mod = "Int")))) 
  ...
  lnCVR_mods_D__pers_0.5 <- pers_0.5[["lnCVR"]] 
  
  #extract PR
  MLMA_estimates_SMD_pers_D_0.5 <- plyr::ldply(lapply(smd_mods_D_pers_0.5, 
                           function(x) print(mod_results(x, mod = "Int")))) 
  MLMA_estimates_lnCVR_pers_D_0.5 <- plyr::ldply(lapply(lnCVR_mods_D__pers_0.5, 
                              function(x) print(mod_results(x, mod = "Int")))) 
  ...
  lnCVR_mods_D__pers_0.3 <- pers_0.3[["lnCVR"]] 
  
  #extract PR
  MLMA_estimates_SMD_pers_D_0.3 <- plyr::ldply(lapply(smd_mods_D_pers_0.3, 
                           function(x) print(mod_results(x, mod = "Int")))) 
  MLMA_estimates_lnCVR_pers_D_0.3 <- plyr::ldply(lapply(lnCVR_mods_D__pers_0.3, 
                              function(x) print(mod_results(x, mod = "Int")))) 
  
    # Pers*SSD traits models
  ...
  # pers * ssd mods - rho=0.8
  ssd_0.8 <- fit_int_MLMAmodel_D_pers_ssd(pers_new, phylo_vcv, D_matrices_0.8)
    
  split_taxa <- split(pers_new, pers_new$taxo_group)
    smd_mods_D_pers_ssd_0.8 <- ssd_0.8[["SMD"]] 
  lnCVR_mods_D__pers_ssd_0.8 <- ssd_0.8[["lnCVR"]] 
  
  #extract PR - need to change function structure here
  MLMA_estimates_SMD_pers_ssd_D_0.8 <- plyr::ldply(lapply(smd_mods_D_pers_ssd_0.8, 
                           function(x) print(mod_results(x, mod = "Int")))) 
  MLMA_estimates_lnCVR_pers_ssd_D_0.8 <- plyr::ldply(lapply(lnCVR_mods_D__pers_ssd_0.8, 
                              function(x) print(mod_results(x, mod = "Int")))) 
    
  # pers * ssd mods - rho=0.5
  ssd_0.5 <- fit_int_MLMAmodel_D_pers_ssd(pers_new, phylo_vcv, D_matrices_0.5)
    
  split_taxa <- split(pers_new, pers_new$taxo_group)
    smd_mods_D_pers_ssd_0.5 <- ssd_0.5[["SMD"]] 
  lnCVR_mods_D__pers_ssd_0.5 <- ssd_0.5[["lnCVR"]] 
  
  # pers * ssd mods - rho=0.3
  ssd_0.3 <- fit_int_MLMAmodel_D_pers_ssd(pers_new, phylo_vcv, D_matrices_0.3)
```  

# SSD models with D matrices - subsetted datasets. To make life easier since we have like a million of these models, I'm just going to subset by personality trait and ignore the model summaries for taxa with not enough data to run the contrasts.
```{r}

  # 1. activity
    pers_new_activity <- as.data.frame(pers_new %>%
    filter(personality_trait == "activity")) 
  
  # 2. boldness
    pers_new_boldness <- as.data.frame(pers_new %>%
    filter(personality_trait == "boldness")) 
  
  # 3. aggression
    pers_new_aggression <- as.data.frame(pers_new %>%
    filter(personality_trait == "aggression")) 
 
  # 4. exploration
    pers_new_exploration <- as.data.frame(pers_new %>%
    filter(personality_trait == "exploration")) 
    
    # I don't think any group has enough species for sociality, so we can ignore this trait too

# Pers*SSD traits models - modify this function for each level of rho and for each personality trait
    fit_int_MLMAmodel_D_pers_ssd <- function(data, phylo_vcv, D){

          lnCVR <- mapply(function(x,y,z) metafor::rma.mv(CVR_yi ~ SSD_index, V = CVR_vi, 
           random = list(~1|study_ID, ~1|spp_name_phylo, ~1|obs), 
           R = list(spp_name_phylo=y, obs = z), 
           data = x, test = "t"), x = split_taxa, y = phylo_vcv, z = D, SIMPLIFY = FALSE)

            SMD <- mapply(function(x,y,z) metafor::rma.mv(SMD_yi_flip ~ SSD_index, V = SMD_vi, 
              random = list(~1|study_ID, ~1|spp_name_phylo, ~1|obs), 
              R = list(spp_name_phylo=y, obs = z), 
              data = x, test = "t"), x = split_taxa, y = phylo_vcv, z = D, SIMPLIFY = FALSE) 
...
# 1a activity rho=0.8
ssd_0.8 <- fit_int_MLMAmodel_D_pers_ssd(pers_new_activity, phylo_vcv, D_matrices_0.8)
   
  # Extract the SMD and lnCVR results
    split_taxa <- split(pers_new_activity, pers_new_activity$taxo_group)
    smd_mods_D_activity_ssd_0.8 <- ssd_0.8[["SMD"]] 
    lnCVR_mods_D_activity_ssd_0.8 <- ssd_0.8[["lnCVR"]] 
...
# 4. INVERTS
  # subset dataset
  # a. activity
  invert_activity <- as.data.frame(invert_noscore %>%
   filter(personality_trait == "activity")) 
  # b. boldness
   invert_bold <- as.data.frame(invert_noscore %>%
   filter(personality_trait == "boldness")) 
...
Here we are adding a moderator term for data that are scores to see if they significantly influence the means of our intercept and pers traits models
```{r}
...
# how many effect sizes are scores?
  ...
  # only invert SMD is significantly affected by score data (slope is significantly different from rest of dataset)
  
  # as a check, let's rerun our 3 main models with score data removed for inverts

     # filter out invert scores from pers dataset  
    pers_new <- pers_new %>%
    filter(taxo_group == "invertebrate" & !score == "score") 
    
    dim(pers_new) #good, just lost the 61 score effect sizes!
  
   # rerun intercept model, pers_trait model and SSD models, then all other models
  ...
  phylo_vcv_invert <- phylo_vcv[[3]]
      ...
      #1. intercept mod
  #SMD
    invert_mod1_smd <- rma.mv(SMD_yi_flip ~ 1, V = SMD_vi, 
                                          random = list(~1|study_ID, ~1|spp_name_phylo, ~1|obs), 
                                          R = list(spp_name_phylo=phylo_vcv_invert), control=list(optimizer="optim"), 
                                          test = "t", data = invert_noscore)
  #lncvr
    invert_mod1_lncvr <- rma.mv(CVR_yi ~ 1, V = CVR_vi, 
                                          random = list(~1|study_ID, ~1|spp_name_phylo, ~1|obs), 
                                          R = list(spp_name_phylo=phylo_vcv_invert), control=list(optimizer="optim"), 
                                          test = "t", data = invert_noscore)
     
    #2. MLMR mod with pers traits 
  #SMD
    invert_mod2_smd <- rma.mv(SMD_yi_flip ~ personality_trait-1, V = SMD_vi, 
                                          random = list(~1|study_ID, ~1|spp_name_phylo, ~1|obs), 
                                          R = list(spp_name_phylo=phylo_vcv_invert), control=list(optimizer="optim"), 
                                          test = "t", data = invert_noscore)
  #lncvr
    invert_mod2_lncvr <- rma.mv(CVR_yi ~ personality_trait -1, V = CVR_vi, 
                                          random = list(~1|study_ID, ~1|spp_name_phylo, ~1|obs), 
                                          R = list(spp_name_phylo=phylo_vcv_invert), control=list(optimizer="optim"), 
                                          test = "t", data = invert_noscore)
    
    # what does the dataset look like for invert traits now?
    data.frame(invert_noscore %>%
    group_by(personality_trait) %>%
    summarise(n = n(), studies = length(unique(study_ID)), species = length(unique(spp_name_phylo)))) 
    
      # enough species to look at boldness and activity for SSD interactions
    
     # subset dataset
      invert_noscore <- as.data.frame(invert_noscore %>%
...
#SMD
  invert_mod3_smd <- rma.mv(SMD_yi_flip ~ -1 + personality_trait*SSD_index, V = SMD_vi, 
                                          random = list(~1|study_ID, ~1|spp_name_phylo, ~1|obs), 
                                          R = list(spp_name_phylo=phylo_vcv_invert), control=list(optimizer="optim"), 
                                          test = "t", data = invert_noscore)

#lnCVR
 invert_mod3_lncvr <- rma.mv(CVR_yi ~ -1 + personality_trait*SSD_index, V = CVR_vi, 
                                            random = list(~1|study_ID, ~1|spp_name_phylo, ~1|obs), 
                                            R = list(spp_name_phylo=phylo_vcv_invert), control=list(optimizer="optim"), 
                                            test = "t", data = invert_noscore)    
...
We have included precision as a moderator term in our full model (personality trait MLMA model) to see if studies with larger sample sizes (greater precision) are published significantly more often than studies with smaller sample sizes. The logic is that studies with larger sample sizes can have significant results due to chance (more sampling = greater chance of finding something significant) or that journals are more likely to publish studies with significant results and large sample sizes. 
...
# 1. funnel plot for all our effect sizes?
  # looks for asymmetry in effect sizes - could be due to heterogeneity or publication bias
          res <- residuals(model) #residuals extractor?
...
# precision is significant only for invertebrates (SMD but not lnCVR) - makes sense since inverts often have the largest sample sizes
  # now that we've removed score data from inverts, does the significant precision slope go away?
  invert_pubbias_lnCVR <- metafor::rma.mv(CVR_yi ~ -1 + personality_trait + precisionlncvr, V = CVR_vi, random = list(~1|study_ID, ~1|spp_name_phylo, ~1|obs), R = list(spp_name_phylo=phylo_vcv_invert), test = "t", data = invert_noscore)
  
  invert_pubbias_SMD <- metafor::rma.mv(SMD_yi_flip ~ -1 + personality_trait + precisionSMD, V = SMD_vi, random = list(~1|study_ID, ~1|spp_name_phylo, ~1|obs), R = list(spp_name_phylo=phylo_vcv_invert), test = "t", data = invert_noscore) 
  
  
   # extract residuals from model to plot in funnel - not working because it's a list - work out how to do this one!!!
  res_smd <- rstandard(smd_mods_pubbias) #SMD
  res_lncvr <- rstandard(lnCVR_mods_pubbias) #lnCVR
  
  # extract residuals from all the models at the same time using modified function and split by taxo group
   taxa_list <- split(data, data$taxo_group)
   persmodel_res <- mapply(function(x, y) fit_MLMA_reg_models_studytype(x, y), x = taxa_list, y = phylo_vcv)
   res_SMD <- plyr::ldply(lapply(smd_mods_pubbias, function(x) print(mod_results(x, mod = "Int"))))
  
  # funnel it
  #SMD
   funnel(x = res_smd, vi = pers_new$SMD_vi, yaxis = "seinv") 
...
# also need ggimage to add silhouettes to trees
library(ggimage)
...
# birds
  ...
  # matrix    
    birds_matrix <- data.matrix(pers_bird) #yay!
...
# need to include legend titles (ssd index for nodes, n effect sizes for pers traits heat map) 
  # will also need to indicate where species have been substituted** important

# ideally, the colours of the pers trait heatmap would match the colours in the orchard plots for easy visualisation 
```

# putting bird plots together
```{r}
# plots to use:
# "birdphylo.pdf"
# bird_SMD
...
# Bird lnCVR
bird_lncvr <- orchard_plot(lnCVR_mods_pers[[1]], mod = "personality_trait", xlab = "lnCVR", angle = 45, alpha = 0.5, N = "none", transfm = "none")

# bird smd
bird_SMD <- orchard_plot(smd_mods_pers[[1]], mod = "personality_trait", xlab = "SMD", angle = 45, alpha = 0.5, N = "none", transfm = "none") 

# window size - birds and mammals will have plots side by side
dev.new(width=9,height=4,noRStudioGD = TRUE)

# trying ggarrange()
bird_effects <- ggarrange(bird_SMD, bird_lncvr, nrow = 1, align = "hv", labels = "a") # still can't get rid of those axis labels
    # just uses the 1 label ???

# using patchwork for comparison
  bird_SMD + bird_lncvr + plot_layout(guides = 'collect') + plot_annotation(tag_levels = 'a')
```
...
# orchard plots
# smd
mammal_SMD <- orchard_plot(smd_mods_pers[[4]], mod = "personality_trait", xlab = "SMD", angle = 45, alpha = 0.5, transfm = "none")
#lncvr
mammal_lncvr <- orchard_plot(lnCVR_mods_pers[[4]], mod = "personality_trait", xlab = "lnCVR", angle = 45, alpha = 0.5, transfm = "none")

# putting the orchard plots together
  # window size
dev.new(width = 10, height = 5, noRStudioGD = TRUE)

# collect the precision legend to the side to make the plots clear
mammal_SMD + mammal_lncvr + plot_layout(guides = 'collect')
ggsave("mammal_orchards.pdf", dpi = 300)
```
...
rep_plot <- gheatmap(p3, rep_matrix, offset = 25, width = 2,
  ...
  # orchard plots
  # smd
reptile_SMD <- orchard_plot(smd_mods_pers[[5]], mod = "personality_trait", xlab = "SMD", angle = 45, alpha = 0.5, transfm = "none")
  # lncvr
reptile_lncvr <- orchard_plot(lnCVR_mods_pers[[5]], mod = "personality_trait", xlab = "lnCVR", angle = 45, alpha = 0.5, transfm = "none")  
```
...
# make a matrix of effect sizes (n) for each species for each personality trait to add to our plot!
  ...
  # orchard plots
#smd
  fish_SMD <- orchard_plot(smd_mods_pers[[2]], mod = "personality_trait", xlab = "SMD", angle = 45, alpha = 0.5, transfm = "none")  
#lncvr
  fish_lncvr <- orchard_plot(lnCVR_mods_pers[[2]], mod = "personality_trait", xlab = "lnCVR", angle = 45, alpha = 0.5, transfm = "none")
...
# orchard plots
# smd
invert_SMD<- orchard_plot(smd_mods_pers[[3]], mod = "personality_trait", xlab = "SMD", angle = 45, alpha = 0.5, transfm = "none")

# lncvr
invert_lncvr<- orchard_plot(lnCVR_mods_pers[[3]], mod = "personality_trait", xlab = "lnCVR", angle = 45, alpha = 0.5, transfm = "none")
```

# patching figures together - trying ggarrange and patchwork to see which works best

```{r}
# packages to use: ggarrange, patchwork, ggpubr (uses ggarrange function)

# set window size
dev.new(width=11.69,height=8.27,noRStudioGD = TRUE) #A4 size

# use patchwork to set size limits to plots
# mammals
  #mammal_plot
  #mammal_SMD
  #mammal_lncvr
...
# inverts
  #invertplot is the phylo
  #invert_SMD SMD orchard plot
  #invert_lncvr lncvr orchard plot

# want phylo on top (centred if possible) with SMD and lncvr next to each other underneath (taller rather than wider would look better), give each group it's own letter so we can tell who's who

invertplot - invert_SMD + invert_lncvr # not a grid shape so doesn't really work
...
GGGGHHHH
"
mammal_plot + mammal_SMD + mammal_lncvr + 
  bird_plot + bird_SMD + bird_lncvr + 
  rep_plot + reptile_SMD + reptile_lncvr +
  fish_plot + fish_SMD + fish_lncvr + 
  invertplot + invert_SMD + invert_lncvr +  
  plot_layout(design = layout)

# let's see if we can grab the legends from heatmap plot to place beside phylogeny
(rep_phylo / bird_SMD | bird_lncvr) + plot_layout(guides = 'collect')  plot_annotation(tag_levels = 'a') # need to work on this one - where it collects to

# collecting repetitive guides to the side so we can trim them off in an edit
dev.new(width = 9, height = 4, noRStudioGD = TRUE)
bird_SMD + bird_lncvr + guide_area() + plot_layout(guides = 'collect')
ggsave("bird_orchards.pdf", dpi = 300)

# somewhat better but still enormous 
dev.new(width = 10, height = 4, noRStudioGD = TRUE)
bird_SMD + bird_lncvr
ggsave("bird_orchards.pdf", dpi = 300)

# try to plot phylo with the legend off the phylo - should make it smaller

# best sizes for things:
 #phylo (birds and mammals)
dev.new(width=7, height=6,noRStudioGD = TRUE)
...
# orchard plots
dev.new(width=10,height=5,noRStudioGD = TRUE) 
  # invert_SMD + invert_lncvr + plot_layout(guides = 'collect') # use the plot_layout bit to collect the precision legends at the end
```
