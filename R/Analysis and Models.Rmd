...
# check for differences between pers and bodysize "species names" columns before joining them
  ...
  setdiff(pers$species_name, bodysize$species_name) 
      #Brachyraphis episcopi is in pers but not bodysize - spelling error now fixed

# Merge spp_names columns
  pers <- merge(x = pers,
                y = bodysize[,c("species_name", "SSD_index")],
                by="species_name", all.x=TRUE,  no.dups = TRUE)
  
# Just select the relevant columns for now to make life easier
  pers_new <- pers %>% 
          select(study_ID, year, species_name, SSD_index, taxo_group, data_type, personality_trait, male_n, male_mean, m_SE_to_SD, female_n, female_mean, f_SE_to_SD, depend, directionality, spp_name_phylo)
...
# adjusted models used for SSD:
  # SSD (not scaled), no interaction term
    lnCVR <- metafor::rma.mv(CVR_yi ~ -1+ personality_trait + SSD_index, V = CVR_vi, random = list(~1|study_ID, random~1|spp_name_phylo, ~1|obs), R = list(spp_name_phylo=phylo_vcv), data = data)

    SMD <- metafor::rma.mv(SMD_yi_flip ~ -1 + personality_trait + SSD_index, V = SMD_vi, random = list(~1|study_ID, ~1|spp_name_phylo, ~1|obs), R = list(spp_name_phylo=phylo_vcv), data = data)

```

# 8. We can also have a look at fitting a model that estimates an interaction between mating system (a second proxy for SSD) and personality trait
...
# NOTE Lauren: Seems to be some problems here, probably just lots of missing data. You're probably just going to have to select the taxon group, corresponding tree and subset the data and selectively run a subset of models. 
  ...
  if(rerun_models == TRUE){
      MLMR_models_pers_mating <- meta_model_fits(pers_mate, phylo_vcv, type = "pers_mate")
...
# first bunch of models kept terminating with the error: There are rows/columns in the 'R' matrix for 'spp_name_phylo' for which there are no data.Error in metafor::rma.mv(CVR_yi ~ parental_care - 1, V = CVR_vi, random = list(~1 |  : Processing terminated since k <= 1.
# Let's try to run these models by dropping reptiles and all NAs and pruning the phylo trees accordingly

# Drop missing data and reptiles  
    pers_new_matingsystem <- as.data.frame(pers_new %>%
    filter(!is.na(mating_system) & !taxo_group == "reptilia"))

  # Check that this has been done correctly. No reptiles and no NA levels 
      pers_new_matingsystem %>%
      group_by(taxo_group, mating_system) %>%
      summarise(n = n(), studies = length(unique(study_ID)))  
      
# polyandrous and polygynous insects only have data from 1 study, let's try dropping those to see if the model works without them
    #pers_new_matingsystem %>%
    #filter(!taxo_group == "invertebrate") | (!mating_system == "polyandrous" & !mating_system == "polygynous")
    
# Trim trees to remove species we lost as a result of NAs
    pers_new_matingsystem_list <- split(pers_new_matingsystem, pers_new_matingsystem$taxo_group, drop = TRUE)

     pruned_trees_MS <- mapply(x = pers_new_matingsystem_list, 
                             y = trees[-5], # drop out reptiles
                             function(x,y) tree_checks(x,y, "spp_name_phylo", type = "prune"))

      # Check that this has been done correctly
        checks <- mapply(x = pers_new_matingsystem_list, 
                             y = pruned_trees_MS, 
                             function(x,y) tree_checks(x,y, "spp_name_phylo", type = "checks"))
        
      for(i in colnames(checks)){
            print(i)
            print(checks[,i] )
      }

# Extract the phylogenetic correlation matrices
            phylo_vcv_MS <- lapply(pruned_trees_MS, function(x) vcv(x, corr = TRUE))      
      
# Rerun the model
     MLMR_models_matingsys <- meta_model_fits(pers_new_matingsystem, phylo_vcv_MS, type = "pers_mate")
            
# Extract the SMD and lnCVR results
    smd_mods_pers_mating <- MLMR_models_pers_mating["SMD",]
  lnCVR_mods_pers_mating <- MLMR_models_pers_mating["lnCVR",]
  
# Extract combined estimates
 MLMA_estimates_SMD_mating <- plyr::ldply(lapply(smd_mods_pers_mating, function(x) print(mod_results(x, mod = "pers_mate"))))
 MLMA_estimates_lnCVR_mating <- plyr::ldply(lapply(lnCVR_mods_pers_mating, function(x) print(mod_results(x, mod = "pers_mate"))))
...
# Get the combined estimates from them all
   MLMA_estimates_SMD_par_care <- plyr::ldply(lapply(smd_mods_par_care, function(x) print(mod_results(x, mod = "parent_care"))))
 MLMA_estimates_lnCVR_par_care <- plyr::ldply(lapply(lnCVR_mods_par_care, function(x) print(mod_results(x, mod = "parent_care"))))
...
# Fish lnCVR
orchard_plot(lnCVR_mods_pers[[2]], mod = "personality_trait", xlab = "log Coefficient of Variance (lnCVR)", alpha = 0.5, 
             transfm = "none")
```
