...
gheatmap(p, birds_matrix, offset = 40, 
...
# window size (A4?)
dev.new(width=8.27,height=11.69,noRStudioGD = TRUE)
...
p2 <- ggtree(pruned.mammaltree, branch.length='none', layout='circular') %<+% mammal_data + 
  geom_tippoint(aes(color=SSD_index)) + 
  scale_color_gradient2(midpoint = 0, low = "red3", mid = "seashell2", high = "deepskyblue2") + 
  geom_tiplab2(align=T, linetype=NA, size=2, offset=8, hjust=0.5) +
...
# make a matrix of effect sizes (n) for each species for each personality trait to add to our plot!
  ...
  # add the heatmap data to our plot
gheatmap(p2, mammal_matrix, offset = 13, 
         low = "white", high = "mediumseagreen", color=NULL, 
         colnames_position="bottom", 
         colnames_angle=85, colnames_offset_y = 1, 
         hjust=0, font.size=2.8) 
```
...
rownames(rep_data) <- pruned.reptree$tip.label
 ...
 # tree structure (cladogram, circular)
p3 <- ggtree(pruned.reptree, layout='fan', open.angle=120) %<+% rep_data + 
  geom_tippoint(aes(color=SSD_index)) + 
  scale_color_gradient2(midpoint = 0, low = "red3", mid = "seashell2", high = "deepskyblue2") + 
  geom_tiplab2(align=T, linetype=NA, size=2, offset=8, hjust=0) # fan shape because we only have 10 species
...
# make a matrix of effect sizes (n) for each species for each personality trait to add to our plot!
  ...
  # add the heatmap data to our plot
        gheatmap(p3, rep_matrix, offset = 20, width = 1,
         low = "white", high = "mediumseagreen", color=NULL, 
         colnames_position="bottom", 
         colnames_angle=90, colnames_offset_y = 1, 
         hjust=0, font.size=2.8) #just not aligning properly 
```
...
# make a matrix of effect sizes (n) for each species for each personality trait to add to our plot!
  ...
  # add the heatmap data to our plot
gheatmap(p4, fish_matrix, offset = 13, 
         low = "white", high = "mediumseagreen", color=NULL, 
         colnames_position="bottom", 
         colnames_angle=45, colnames_offset_y = 1, 
         hjust=0.5, font.size=2.8) 
```
...
p5 <- ggtree(pruned.inverttree, branch.length='none', layout='circular') %<+% invert_data + 
  geom_tippoint(aes(color=SSD_index)) + 
  scale_color_gradient2(midpoint = 0, low = "red3", mid = "seashell2", high = "deepskyblue2") + 
  geom_tiplab2(align=T, linetype=NA, size=2, offset=7, hjust=0.5) +
  theme(legend.position = "right")
...
# make a matrix of effect sizes (n) for each species for each personality trait to add to our plot!
  ...
  # add the heatmap data to our plot
inv_plot <- gheatmap(p5, invert_matrix, offset = 12, 
         low = "white", high = "mediumseagreen", color=NULL, 
